...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/fantenucHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    new_priority = IntegerField("What is the new priority of this item?", validators=[Required()])
    submit = SubmitField('Update')
...
def delete(lst):
    ...
    # return redirect(url_for('all_lists', form=form, todo_lists=lsts))
    return redirect(url_for('all_lists'))
